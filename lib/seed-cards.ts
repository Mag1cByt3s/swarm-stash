// The built-in lore card set, seeded into the `cards` table on startup so the
// catalog lives in the database (and can be edited via the admin page) instead
// of being hard-coded in source. This file is the only bootstrap source for
// cards; everything else reads them back out of the DB at runtime.
//
// seedCards() is idempotent: it only inserts cards whose id is missing, so it
// never overwrites edits an admin made to an existing card, and it's safe to
// run on every boot.
...
export function seedCards(): void {
  for (const c of SEED_CARDS) {
    if (!store.getCard(c.id)) store.createCard(c);
  }
}
